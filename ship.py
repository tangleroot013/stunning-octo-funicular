...
_CONVENTIONAL_RE = re.compile(r'^(feat|fix|docs|style|refactor|perf|test|chore)(\([\w-]+\))?!?: .+')


def _validate_commit_message(msg):
    """Return True if msg follows Conventional Commits, else log and abort."""
    if not _CONVENTIONAL_RE.match(msg):
        _fail("commit message does not follow Conventional Commits")
        _hint("expected format: type(scope?): description  e.g. 'fix(ship): handle empty diff'")
        return False
    return True
...
def _sync_lockfile():
    ...
    if not diff.stdout:
        return
    changed = set(diff.stdout.strip().splitlines())
    if Path("package-lock.json").exists() and "package.json" in changed:
        _run("npm install", capture=False)
        _run("git add package-lock.json")
        _info("synced package-lock.json")
    if Path("Pipfile.lock").exists() and "Pipfile" in changed:
...
def _type_check():
    ...
    print("\n=== type check ===")
    venv_python = _get_venv_python()
    result = _run([venv_python, "-m", "mypy", "src", "--explicit-package-bases"], check=False)
    if result.returncode != 0:
...
def _forbidden_size_check():
    ...
    offenders = []
    for root, _dirs, files in os.walk("."):
        if {".git", "node_modules", ".venv", "__pycache__"} & set(Path(root).parts):
            continue
        for f in files:
            p = Path(root) / f
            try:
